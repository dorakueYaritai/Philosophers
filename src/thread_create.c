...
/*   Updated: 2023/03/23 18:16:02 by kakiba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	writer_create(t_queue *queue, pthread_t *th_id, int thread_num)
{
	// if (pthread_create(&th_id[writer_id], NULL, &routine_init, &queue) != 0)
	if (pthread_create(&th_id[thread_num - 1], NULL, &writer_init, queue) != 0)
		return (ERROR);
	return (SUCCESS);
}
