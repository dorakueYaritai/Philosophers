...
int	be_kind_neighbors(t_philo *philo, t_fork *fork, t_fork *had)
{
	...
	else if (had_side == false && fork_side == true)
	{
		...
		// if (is_neighbor_hungry(philo, fork) == true)
		// {
		// 	put_fork(philo, fork);
		// 	usleep(10);
		// 	take_fork(philo, fork, had);
		// }
		// if (is_neighbor_hungry(philo, had) == true)
		// {
		// 	put_fork(philo, had);
		// 	usleep(10);
		// 	take_fork(philo, fork, NULL);
		// }
...
bool	check_am_i_dead(t_philo *philo)
{
	...
	// pthread_mutex_lock(&philo->dead_info->mutex);
	while (ft_pthread_mutex_trylock(&philo->dead_info->mutex) == false)
		;
	data = philo->dead_info->is_death;
	ft_pthread_mutex_unlock(&philo->dead_info->mutex);
	// pthread_mutex_unlock(&philo->dead_info->mutex.stuff);
	return (data);
}
